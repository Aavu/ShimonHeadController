package ShimonHeadController.innards.util;
//
...
//chances are you never need to interact with this.  just use the ResourceLocator in ShimonHeadController.innards.util.
 ...
 * specify <code>InnardsDefaults</code> values within your app bundle or jar that will seed the ShimonHeadController.innards defaults on a new machine,
...
//	public static boolean LOOK_ONLY_IN_MAIN_BUNDLE= false;
	...
	public String getPathForResource(String resource, boolean mustExist)
	{
		...
		if(path == null && !mustExist){
			...
			if(contentRootPath == null || contentRootPath.length()==0){
				System.out.println("No Value set for ContentRootPath in ShimonHeadController.innards defaults, but it is needed.  please set it");
			}else{
	...
	public String findResourceFromPaths(String resourceName, String[] paths)
	{
		...
		if (paths == null)
		{
			System.out.println("WinXPSpecificResourceLocator has no paths to search... try putting some in ShimonHeadController.innards properties.");
			return null;
		}
